...
// Load configuration from firebase-applet-config.json as fallback/default
...
// Firebase configuration using environment variables or applet config file
...
if (isFirebaseConfigured) {
  try {
    ...
    console.log("Firebase initialized successfully on server backend with databaseId:", databaseId || "(default)");
  } catch (err) {
    console.error("Failed to initialize Firebase on server backend:", err);
  }
} else {
  console.log("Firebase environment variables and config are missing on the server. Falling back to local JSON store.");
}
