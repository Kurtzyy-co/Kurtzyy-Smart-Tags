...
const parseDOB = (dobString?: string): Date | null => {
  ...
  // If DD/MM/YYYY
  if (/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(dobString)) {
    const parts = dobString.split("/");
...
const calculateAge = (dobString?: string): string => {
  ...
  if (birthDate > today) return "0 Days";
...
export default function App() {
  ...
  const [currentProfileId, setCurrentProfileId] = useState<string>(() => {
    // Determine profile ID from URL path, query parameter, or hash (e.g., /profile-1, ?id=profile-1, or #profile-1)
    const path = window.location.pathname.replace(/^\/+|\/+$/g, "");
    const searchParams = new URLSearchParams(window.location.search);
    const queryId = searchParams.get("profile") || searchParams.get("id");
    if (queryId) return queryId;
    const hashId = window.location.hash.replace(/^#+/, "");
    if (hashId && !hashId.includes(".") && !hashId.startsWith("api")) {
      return hashId;
    }
  ...
  const [urlFormat, setUrlFormat] = useState<"clean" | "query" | "hash">("query");
  ...
  const fetchProfilesList = async () => {
    if (isFirebaseConfigured) {
      try {
        ...
        const profilesMap: Record<string, TagProfile> = {};
        querySnapshot.forEach((docSnap) => {
          profilesMap[docSnap.id] = docSnap.data() as TagProfile;
        });
  ...
  const handleCreateNewProfile = async (e: React.FormEvent) => {
    ...
    try {
      if (isFirebaseConfigured) {
        await setDoc(doc(db, "profiles", sanitizedId), newProfile);
  ...
  const handleDeleteProfile = async () => {
    ...
    try {
      ...
      const updatedAllProfiles = { ...allProfiles };
      delete updatedAllProfiles[profile.id];
      setAllProfiles(updatedAllProfiles);

      // Transition smoothly to another profile or fallback
      const remainingIds = Object.keys(updatedAllProfiles);
      const nextProfileId = remainingIds.length > 0 ? remainingIds[0] : "kurtzy-huzz";

      setCurrentProfileId(nextProfileId);
      setShowDeleteConfirm(false);

      // Clean or update query param / hash in location bar
      if (window.location.hash) {
        window.location.hash = `#${nextProfileId}`;
      } else if (window.location.search) {
        const params = new URLSearchParams(window.location.search);
        if (params.has("id")) {
          params.set("id", nextProfileId);
        ...
        window.history.replaceState({}, "", `/${nextProfileId}`);
      }
    } catch (err) {
            ...
            <AnimatePresence mode="wait">
              {isAdmin ? (
                ...
                >
                  ...
                  <div className="p-5 bg-blue-500/5 border border-blue-500/15 rounded-xl space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[9px] font-mono text-emerald-400 bg-emerald-500/10 px-1.5 py-0.5 rounded font-bold">LIVE REDIRECT</span>
                    </div>
                    ...
                    <div className="flex border-b border-slate-800/80">
                      <button
                        ...
                        className={`flex-1 pb-2 text-xs font-mono font-medium transition-colors border-b-2 text-center ${
                          urlFormat === "query" 
                    ...
                    {/* Display current active URL based on selected format */}
                    <div className="flex items-center gap-2 bg-slate-950 p-2 rounded-lg border border-slate-800">
                      <span className="font-mono text-xs text-slate-300 flex-1 truncate select-all px-1">
                        {urlFormat === "clean" && `${window.location.origin}/${profile.id}`}
                        {urlFormat === "query" && `${window.location.origin}/?id=${profile.id}`}
                        {urlFormat === "hash" && `${window.location.origin}/#${profile.id}`}
                      </span>
                      <button
                        onClick={() => {
                          const valToCopy = 
                        ...
                        className="px-3 py-1 bg-blue-600/20 hover:bg-blue-600 text-blue-300 hover:text-white text-[11px] font-bold rounded transition-colors whitespace-nowrap"
                      >
                        {isCopied ? "Copied!" : "Copy URL"}
                      </button>
                    </div>

                    <div className="text-[11px] text-slate-400 bg-slate-950/40 p-3 rounded-lg border border-slate-900 leading-normal space-y-1.5">
                      {urlFormat === "query" && (
                        <p>
                          <strong className="text-emerald-400">✓ Highly Recommended:</strong> This uses standard query parameters. It works flawlessly out-of-the-box on Vercel, Netlify, and GitHub Pages without any setup!
                        </p>
                      )}
                      {urlFormat === "hash" && (
                        <p>
                          <strong className="text-emerald-400">✓ Highly Recommended:</strong> This uses hash-based client routing. Perfect for static hosts (like Vercel) and prevents any 404 errors during direct browser refreshes.
                        </p>
                      )}
                      {urlFormat === "clean" && (
                        <p>
                          <strong className="text-amber-400">⚠️ Host Configuration Required:</strong> To use clean paths on static platforms like Vercel, make sure your project contains the <code className="text-blue-300 bg-slate-900 px-1 rounded">vercel.json</code> file we automatically created for you in the repository root. This tells Vercel's edge network to rewrite path requests to the main app file.
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div>
                        <label className="block text-[11px] font-mono text-slate-400 uppercase mb-1.5">Date of Birth (DD/MM/YYYY)</label>
                        <input 
                          type="text" 
                          ...
                          onChange={(e) => {
                            ...
                            // Auto-insert slash after 5 characters (DD/MM/)
                            ...
                            const computedAge = calculateAge(sanitized);
                            setProfile(prev => ({ 
                              ...prev, 
                              dob: sanitized,
                        ...
                        />
                        {profile.dob && (
                          <div className="text-[11px] font-medium font-mono mt-1.5">
                            {parseDOB(profile.dob) ? (
                              <div className="text-emerald-400 flex items-center gap-1">
                                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                                <span>Calculated Age: {calculateAge(profile.dob)}</span>
                              </div>
                            ) : (
                              <div className="text-rose-400 flex items-center gap-1">
                                <span className="w-1.5 h-1.5 rounded-full bg-rose-500" />
                                <span>Please enter a valid date (DD/MM/YYYY)</span>
                              </div>
                            )}
                          </div>
                        )}
                      ...
                      {showDeleteConfirm ? (
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            disabled={isDeleting}
                            className="flex items-center gap-1.5 px-4 py-2 bg-rose-600 hover:bg-rose-500 disabled:opacity-50 text-white font-bold text-xs rounded-xl shadow-lg shadow-rose-600/10 transition-all"
                          >
                        ...
                        <button
                          ...
                          className="flex items-center gap-1.5 px-4 py-2.5 bg-slate-950 border border-rose-500/20 hover:border-rose-500 hover:bg-rose-950/30 text-rose-400 font-bold text-xs rounded-xl transition-all"
                          title="Permanently remove this child profile"
                        >
